...
export default async function BrandTrashPage() {
  const author = await getAuthor();
  const items = await listTrashed("brand");
    ...
    <>
      ...
      <main className="mx-auto max-w-3xl px-6 py-10">
        ...
        <div className="mb-6">
          ...
          <h1 className="mt-3 font-display text-display-md font-semibold text-ink">Brands trash</h1>
          <p className="mt-2 text-ink-soft">
            Deleted brands are kept here so you can bring them back. Delete forever to remove them
            permanently.
          </p>
        </div>
        <TrashList items={items} workType="brand" />
      </main>
    </>
  );
}
